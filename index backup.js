...
var privateKey = fs.readFileSync('/opt/cer/privatekey.pem', 'utf8');
    ...
    if (msg.type === 'DIEUKHIEN' && Array.isArray(msg.Data)) { //nhan ban tin dieu khien cong suat
      ...
      wss.clients.forEach((client) => {
        if (client !== ws && client.readyState === WebSocket.OPEN) {
          ...
          //let message = {
          //tenthietbi: _data[i][0],
          //idthietbi: _data[i][1],
          //id_cambien: _data[i][2],
          //value: _data[i][3],
          //time: _data[i][4]
          //};
...
const ws = new WebSocket('wss://smartgrid.ifc.com.vn:6332');
    ...
    .then((res) => {
      ...
      res.data.forEach((device) => {
        try {
          ...
          obj.index = 0;
          arr_ipAddresses.push(obj);
        } catch (e) {
          console.error("Parse error:", e);
        }
      });

      console.log(arr_ipAddresses)
      initConnect(arr_ipAddresses);
    })
    .catch((err) => {
      console.error(err);
    });
...
function initConnect(ipAddresses) {
  ipAddresses.forEach((ipAddress) => {
    connectToHardwareDevice(ipAddress);
  });
}
...
function connectToHardwareDevice(ipAddress) {
  const client = net.createConnection(
    ...
    () => {
      console.log(`Connected to ${ipAddress.ip}:${ipAddress.port}`);
      ...
      if (!ipAddConnected.some(i => i.ip === ipAddress.ip && i.port === ipAddress.port)) {
        ipAddConnected.push(ipAddress);
        clients.push(client);
      }
    }
  );
  ...
  client.on('timeout', () => {
    console.log('Connection timed out! ' + ipAddress.ip);
    let message = {
      type: "DISCONNECT",
      Data: ipAddress
    };
    //sendToWS(message);
    client.destroy(); // Hủy kết nối
    retry();
  });
  ...
  client.on("close", () => {
    clearInterval(client.heartbeat); // ❗ rất quan trọng
  });
  client.on("end", () => {
    console.log(`Disconnected from ${ipAddress.ip}`);
    removeItem(ipAddConnected, ipAddress);
    removeItem(clients, client);
    retry();
    //console.log(JSON.stringify(ipAddConnected));
  });
  client.on("error", () => {
    console.log(`Không kết nối được với ${ipAddress.ip}`);
    let message = {
      type: "DISCONNECT",
      Data: ipAddress
    };
...
function removeItem(arr, item) {
  return arr.filter(i => !(i.ip === item.ip && i.port === item.port));
}
